...
def train_model(model, optimizer, config, train_dataset, val_dataset, test_dataset):
    ...
    # pdb.set_trace()
    train_losses = []
    for i in range(config.epoch_start, config.epochs+1):
        progress_bar = tqdm.tqdm(
            total=len(train_dataloader), desc="epoch % 3d" % (i)
        )

        epoch_train_losses = []
        for bid, batch in enumerate(train_dataloader):

            # flops, params = profile(model, inputs=(batch, train_pairs))
            # pdb.set_trace()
        ...
        progress_bar.close()
        progress_bar.write(f"epoch {i} train loss {np.mean(epoch_train_losses)}")
...
def evaluate(model, dataset, config):
    ...
    print(result)
    txt_save = './' + config.save_path + '/' + 'record.txt'
