...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, q, k, v=None):### 40*32*768, 40*257*768
        # pdb.set_trace()
        if v is None:
            v = k
        q = q + self.cross_attn(q, k, v)
...
class Troika(nn.Module):
    ...
    def __init__(self, config, attributes, classes, offset):
        ...
        # --------------------------------------------- novel module ---------------------------------------------
        self.cmt_fusion = nn.ModuleList([CrossAttentionLayer(output_dim, output_dim//64, self.cross_attn_dropout) for _ in range(1)])
        # self.img_fusion = nn.Linear(3*768, 768, True)
        # --------------------------------------------------------------------------------------------------------
        self.lamda = nn.Parameter(torch.ones(output_dim) * config.init_lamda)
        self.patch_norm = nn.LayerNorm(output_dim)

        self.feature_fusion = nn.Parameter(torch.tensor(config.fusion))
    ...
    def encode_text(self, token_ids, token_tensors=None, enable_pos_emb=False):
        # encoded_feature_one, encoded_feature_two = [], []
        # sub_batch_size = 10
        # for i in range(token_tensors.shape[0] // sub_batch_size):
        #     sub_token_tensors = token_tensors[i*sub_batch_size:(i+1)*sub_batch_size]
            
        #     sub_feature_one, sub_feature_two = self.text_encoder(token_ids, sub_token_tensors, enable_pos_emb)
        #     encoded_feature_one.append(sub_feature_one)
        #     # encoded_feature_two.append(sub_feature_two)

        # # res = self.text_encoder(token_ids, token_tensors[:100], enable_pos_emb)
        # encoded_feature_one = torch.cat(encoded_feature_one, dim=0)
        # # encoded_feature_two = torch.cat(encoded_feature_two, dim=1)
        # print(encoded_feature_one.shape)
        # return encoded_feature_one.float(), encoded_feature_two
    ...
    def loss_calu(self, predict, target):
        ...
        if len(target) == 4:
            ...
            _,_,_,batch_attr, batch_obj, batch_target = target
    ...
    def forward_for_open(self, batch, text_feats):
        ...
        batch_img_woback = batch[1].cuda()
        batch_img_depth = batch[2].cuda()


        b = batch_img.shape[0]
        # l, _ = idx.shape
        batch_img, batch_patch = self.encode_image(batch_img.type(self.clip.dtype))
        batch_img_woback, batch_patch_woback = self.encode_image(batch_img_woback.type(self.clip.dtype)) ### 40*257*768
        batch_img_depth, batch_patch_depth = self.encode_image(batch_img_depth.type(self.clip.dtype))
        ...
        batch_patch_woback = self.patch_norm(batch_patch_woback)
        batch_patch_depth = self.patch_norm(batch_patch_depth)


        for layer in self.cmt_fusion:
            # batch_patch = layer(batch_patch_depth, batch_patch_woback, batch_patch)
            # batch_patch = layer(batch_patch_depth, batch_patch)
            batch_patch = layer(self.feature_fusion * batch_patch_depth + (1-self.feature_fusion) * batch_patch_woback, self.feature_fusion * batch_patch_woback + (1-self.feature_fusion) * batch_patch_depth, batch_patch)
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            # scaled_lambda = torch.sigmoid(self.lamda)
            # cmt_text_features = (1 - scaled_lambda) * idx_text_features + scaled_lambda * cmt_text_features.squeeze(1)
    ...
    def forward(self, batch, idx):
        ...
        batch_img, batch_patch = self.encode_image(batch_img.type(self.clip.dtype)) ### 40*257*768
        batch_img_woback, batch_patch_woback = self.encode_image(batch_img_woback.type(self.clip.dtype)) ### 40*257*768
        batch_img_depth, batch_patch_depth = self.encode_image(batch_img_depth.type(self.clip.dtype))



        # batch_img = torch.cat([batch_img, batch_img_woback, batch_img_depth], dim=1)
        # batch_img = self.img_fusion(batch_img)
...
# 
        ...
        token_tensors = self.construct_token_tensors(idx) ### 32*8*768
        # pdb.set_trace()
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            # pdb.set_trace()
            # _text_features = _text_features.float()
            idx_text_features = _text_features / _text_features.norm(
                dim=-1, keepdim=True
            ) ### first, 32*768

            # CMT
            cmt_text_features = idx_text_features.unsqueeze(0).expand(b, -1, -1) ### 40*32*768
            # batch_patch = self.patch_norm(batch_patch)
            ...
            # cmt_text_features = idx_text_features + 0.05 * cmt_text_features.squeeze(1)
