'''
CUDA_VISIBLE_DEVICES=1 python -u download_data/feasibility.py --dataset food172 --dataset_root /data1/sjj/Troika/dataset_root--data_root /data1/sjj/Troika/word_embeddings
'''
import argparse
import os
...
def load_glove_embeddings(vocab):
    ...
    emb_file = os.path.join(config.data_root, 'glove/glove.6B.300d.txt')
    ...
    for k in vocab:
        ...
        if '.' in k:
            ks_old = k.split('.')
            ks = ["spareribs" if item == "sparerib" else item for item in ks_old]
